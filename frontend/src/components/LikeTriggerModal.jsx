...
export default function LikeTriggerModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center" role="dialog" aria-modal="true">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-xl mx-4 bg-tiktok-black border border-tiktok-gray rounded-xl shadow-2xl animate-fadeInUp">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-tiktok-gray">
          <div>
            <h2 className="text-lg font-semibold text-tiktok-white">
              {editingTrigger ? 'Edit Like Trigger' : 'Add Like Trigger'}
            </h2>
            <p className="text-xs text-gray-400 mt-0.5">Automate actions based on like milestones</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 space-y-4">
          ...
          {!editingTrigger && (
            <div>
              <label className="block text-xs font-medium text-tiktok-white mb-2">Quick Setup</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-medium text-tiktok-white mb-1">Like Threshold *</label>
              <input
                type="number"
                min="1"
                placeholder="100"
                className="w-full bg-gray-800 border border-gray-600 px-3 py-2 rounded-lg text-white placeholder-gray-400 focus:border-tiktok-cyan focus:outline-none"
                value={threshold}
                onChange={(e) => setThreshold(Number(e.target.value))}
              />
              <p className="text-[10px] text-gray-500 mt-1">Every N likes</p>
            </div>

            <div>
              <label className="block text-xs font-medium text-tiktok-white mb-1">Keyboard Key *</label>
              <input
                placeholder="e.g., a, space, ctrl+c"
                className="w-full bg-gray-800 border border-gray-600 px-3 py-2 rounded-lg text-white placeholder-gray-400 focus:border-tiktok-cyan focus:outline-none"
                value={key}
                onChange={(e) => setKey(e.target.value)}
                maxLength={20}
              />
              <p className="text-[10px] text-gray-500 mt-1">Simple keys or combos</p>
            </div>

            <div>
              <label className="block text-xs font-medium text-tiktok-white mb-1">Hold Duration (sec)</label>
              <input
                type="number"
                step="0.1"
                min="0"
              ...
              />
              <p className="text-[10px] text-gray-500 mt-1">Typical 1.0s</p>
            </div>
          </div>

          {/* Live Preview */}
          {threshold > 0 && key.trim() && (
            <div className="p-3 bg-tiktok-gray/30 rounded-lg border border-gray-600">
              <h4 className="text-xs font-medium text-tiktok-white mb-2 flex items-center gap-2">
        ...
        {/* Footer */}
        <div className="flex items-center justify-between p-4 border-t border-tiktok-gray">
          <div className="text-[11px] text-gray-500">Press <kbd className="bg-gray-700 px-2 py-0.5 rounded">Esc</kbd> to cancel, <kbd className="bg-gray-700 px-2 py-0.5 rounded">Ctrl+Enter</kbd> to save</div>
          <div className="flex gap-3">
            ...
            <button
              ...
              className="px-3 py-2 bg-tiktok-red hover:bg-tiktok-pink disabled:bg-gray-700 disabled:text-gray-400 text-white rounded-lg transition-colors font-medium"
            >
